...
int open_and_copy_file(const char *source, const char *destination)
{
    ...
    fd_to = open_destination_file(destination, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
...
/**
 * read_and_write_files - Reads from the source file and writes to the destination file.
